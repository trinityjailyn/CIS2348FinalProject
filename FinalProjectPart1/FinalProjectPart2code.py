    ...
    while user != 'q':
        ...
        if user == 'q':
           break
            ...
            # Split the user u_input into individual words
            user = user.split()
            # Flag to check if the u_input is incorrect
            wrong_input = False
            # Iterate over each u_input in user u_input
            for u_input in user:
                # If the u_input is in manufacturers_list and a manufacturer hasn't been selected yet, select it
                if u_input in manufacturers_list:
                   if select_man:
                       # If a manufacturer has already been selected, set wrong_input to True
                       wrong_input = True
                   else:
                      select_man = u_input
                # If the u_input is in item_types and a type hasn't been selected yet, select it
                elif u_input in item_types:
                   if select_item_type:
                      # If a type has already been selected, set wrong_input to True
                      wrong_input = True
                   else:
                      select_item_type = u_input
